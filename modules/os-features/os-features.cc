// os-features: a breadth conformance test for the OS surface the slim kernel
// must keep providing. Phase 0 guardrail (see REFACTORING.md): every phase
// must end with this app still passing, so a cut that breaks a facility the
// real app relies on is caught immediately.
//
// It exercises the facilities the refactoring promises to preserve - threads,
// memory mapping, time, and the libc surface - and nothing it does should ever
// stop working as subsystems are deleted. Each CHECK prints a line; the process
// exits non-zero if any check fails and prints a final tally.
