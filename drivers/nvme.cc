...
// User-queue-only mode: no block device interface needed.
...
static void setup_identify_cmd(nvme_sq_entry_t *cmd, u32 namespace_id,
                               u32 cns) {
  memset(cmd, 0, sizeof(nvme_sq_entry_t));
...
driver::driver(pci::device &pci_dev) : _dev(pci_dev), _msi(&pci_dev) {
  ...
  _qsize = (NVME_IO_QUEUE_SIZE < _control_reg->cap.mqes)
               ? NVME_IO_QUEUE_SIZE
               : _control_reg->cap.mqes + 1;
  ...
  // User-queue-only driver: the controller is initialized but NOT registered as
  // an OSv block device. Consumers (e.g. leanstore) obtain user I/O queues on
  // demand via create_io_user_queue()/osv_create_io_user_queue().
  create_io_user_queue_endpoints();

  // put the current nvme dirver in the linked list
  _next_nvme_driver = driver::prev_nvme_driver;
  // u16 ret;
  // set_number_of_queues(512, &ret);
  driver::prev_nvme_driver = this;
  printf("created device with id %i\n", _id);
}

bool driver::reset_and_destroy_controller() {
  // what we want to do here is reset the controller to a sane state and then
...
int driver::set_number_of_queues(u16 num, u16 *ret) {
  ...
  printf("Queues supported: CQ num=%d, SQ num=%d, MSI/X entries=%d",
...
bool driver::shutdown_controller() {
  ...
  assert(cc.en ==
         1); // 1. If the controller is enabled (i.e., CC.EN is set to ‘1’)
...
int driver::wait_for_controller_shutdown_done() {
  ...
  NVME_ERROR("timeout=%d waiting for shutdown with current status%d type%d",
             timeout, csts.shst, csts.st);
  return ETIME;
}
...
template <typename Q>
void setup_create_io_queue_cmd(Q *create_queue_cmd, int qid, int qsize,
...
template <typename Q>
void setup_delete_io_queue_cmd(Q *delete_queue_cmd, int qid, u8 command_opcode,
                               u64 queue_addr) {
...
void *driver::create_io_interrupt_user_queue(int individual_qsize) {
  ...
  set_interrupt_coalescing(128, 100); // 200us coalescing or >=8 completed
...
int driver::remove_io_user_queue(void *queue) {
  ...
  if (_io_queues.size() > qid) {
    NVME_ERROR("Remove io user queue failed size=%d, id=%d", _io_queues.size(),
               qid);
    return 0;
  }
  ...
  // According to the NVMe spec, the completion queue (CQ) needs to be removed
  // before the submission queue (SQ)
  _admin_queue->submit_and_return_on_completion((nvme_sq_entry_t *)&cmd_cq);
  _admin_queue->submit_and_return_on_completion((nvme_sq_entry_t *)&cmd_sq);
...
int driver::identify_controller() {
  ...
  for (int attempt = 0;; attempt++) {
    ...
    if (res.sc != 0 || res.sct != 0) {
      NVME_ERROR("Identify controller failed nvme%d, sct=%d, sc=%d", _id,
                 res.sct, res.sc);
      return EIO;
    }
    if (data->cqes != 0) {
    ...
    if (attempt >= 60) { // 30s, same order as EBS attach worst cases
      NVME_ERROR("Identify controller returned no data nvme%d", _id);
      return EIO;
    }
    if (attempt == 0) {
      printf("nvme%d: identify data empty, waiting for backing storage\n",
             _id);
    }
    usleep(500 * 1000);
  }
...
int driver::identify_namespace(u32 nsid) {
  ...
  for (int attempt = 0;; attempt++) {
    ...
    if (res.sc != 0 || res.sct != 0) {
      NVME_ERROR("Identify namespace failed nvme%d nsid=%d, sct=%d, sc=%d",
                 _id, nsid, res.sct, res.sc);
      return EIO;
    }
    if (data->ncap != 0) {
      break;
    ...
    if (attempt >= 60) {
      NVME_ERROR("Identify namespace returned no data nvme%d nsid=%d", _id,
                 nsid);
      return EIO;
    }
    if (attempt == 0) {
      printf("nvme%d: namespace %d empty, waiting for backing storage\n", _id,
             nsid);
    }
    usleep(500 * 1000);
  }
...
void driver::register_admin_interrupt() {
  sched::thread *aq_thread =
      sched::thread::make([this] { this->_admin_queue->req_done(); },
                          sched::thread::attr().name(
                              "nvme" + std::to_string(_id) + "_aq_req_done"));
  aq_thread->start();

  assert(msix_register(
      0, [this] { this->_admin_queue->disable_interrupts(); }, aq_thread));
}
...
void driver::enable_msix() {
  ...
  unsigned int vectors_num = _dev.msix_get_num_entries();
...
bool driver::msix_register_io_queue(unsigned iv, unsigned qid) {
  ...
  _msi.assign_isr(vec.get(), [this, qid]() {
...
bool driver::msix_register(unsigned iv,
                           ...
                           // bottom half
                           sched::thread *t, bool assign_affinity) {
  // Mask all interrupts...
  _dev.msix_mask_all();
  _dev.msix_mask_entry(iv);
