...
kvmclock::kvmclock() : _pvclock(get_pvclock_flags()) {
  _wall_time_msr =
      (_new_kvmclock_msrs) ? msr::KVM_WALL_CLOCK_NEW : msr::KVM_WALL_CLOCK;
  ...
  //     printf("kvm clock sync is started.\n");
  //     while (true) {
...
void kvmclock::init_on_cpu() {
  auto system_time_msr =
      (_new_kvmclock_msrs) ? msr::KVM_SYSTEM_TIME_NEW : msr::KVM_SYSTEM_TIME;
...
u64 kvmclock::wall_clock_boot() { return _pvclock.wall_clock_boot(_wall); }
