...
void cpu::on_cpu_up()
{
    // Fire the per-CPU "cpu up" notifiers (clock per-CPU setup, percpu
    // workers, dynamic percpu construction). This must run on the CPU being
    // brought up, since the callbacks initialize cpu::current()'s per-CPU
    // state. It used to be the first action of the load-balancer thread;
    // migration has been removed, so this is now a one-shot called during
    // bringup with no persistent background thread.
    notifier::fire();
}
