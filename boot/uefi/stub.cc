...
// Print a value as 0x-prefixed hex (debug aid for early bring-up on real
// firmware, where only the UEFI console is known-good).
...
// the GAS AddressSpaceId (offset 40) distinguishes system-memory MMIO (0) from
// system I/O (1); the GAS Access Size (offset 43) gives the register access
// width (1=byte, 2=word, 3=dword) - AWS Graviton's 16550 is dword (mmio32).
unsigned char spcr_iface_type = 0xff;
unsigned char spcr_gas_space  = 0xff;
...
uint64_t find_spcr_uart(uint64_t rsdp)
{
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (t[0] == 'S' && t[1] == 'P' && t[2] == 'C' && t[3] == 'R') {
            ...
            spcr_gas_space  = t[40];  // GAS AddressSpaceId (0 = MMIO, 1 = system I/O)
...
extern "C" EFIAPI EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *st)
{
    ...
    // Diagnostic over the (known-good) UEFI console: where is the SPCR UART and
    // what kind is it? Remove once serial bring-up on real hardware is settled.
    print("SPCR uart_base="); print_hex(uart_base);
    print(" iface="); print_hex(spcr_iface_type);
    print(" gas_space="); print_hex(spcr_gas_space);
    print(" access_size="); print_hex(spcr_access_size);
    print("\n");
