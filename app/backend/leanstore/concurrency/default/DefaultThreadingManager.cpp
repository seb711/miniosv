...
// sched.hh references an unqualified ::wait_record (normally forward-declared by
// the includer via lockfree/mutex.hh); declare it so sched.hh compiles here.
struct wait_record;
...
IoChannel& DefaultThreadingManager::execIoChannel()
{
   ...
   return IoInterface::instance().getIoChannel(sched::current_cpu->id);
#else
...
void DefaultThreadingManager::parallelFor(BlockedRange bb, TaskFunction fun, const int tasks, s64 bbgranularity, bool rate_active)
{
   ...
   for (int core_id = 0; core_id < num_cores; core_id++) {
      ...
      workload_generators.emplace_back([=, &fun]() {
         ...
         std::mt19937 gen((unsigned)mean::readTSC());
...
void DefaultThreadingManager::blockingIo(IoRequestType type, char* data, s64 addr, u64 len)
{
   ...
   // Park the current thread until the IO completion callback wakes it.
   std::atomic<void*> waiter_thread{sched::thread::current()};

   UserIoCallback cb;
   cb.callback = [](IoBaseRequest* req) {
      auto* waiter_thread = (std::atomic<void*>*)(req->user.user_data.val.ptr);
      ((sched::thread*)waiter_thread->load(std::memory_order_relaxed))
          ->wake_with_from_mutex([waiter_thread] { waiter_thread->store(nullptr, std::memory_order_release); });
   };
   cb.user_data.val.ptr = &waiter_thread;
   ...
   {
      sched::thread::wait_until([&] { return !waiter_thread.load(std::memory_order_acquire); });
   }
}
