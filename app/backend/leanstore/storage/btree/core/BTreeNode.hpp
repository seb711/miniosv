...
#include <vector>
// -------------------------------------------------------------------------------------
// Intentionally NOT `using namespace std;`. This header is included
// (transitively) by TUs that also pull in OSv kernel headers, and a blanket
// using-directive drags std::mutex into the global namespace, making the
// kernel's `mutex` (lockfree::mutex) ambiguous. Re-export the specific std
// names the btree code uses unqualified instead — but never `mutex`.
using std::array;
using std::atomic;
using std::cerr;
using std::cout;
