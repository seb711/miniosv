...
class BTreeExclusiveIterator : public BTreePessimisticIterator<LATCH_FALLBACK_MODE::EXCLUSIVE>
{
  public:
   ...
   {
      if (!keyFitsInCurrentNode(key)) { dumpLeafOnMismatch("insertInCurrentNode(len)", key); }
   ...
   {
      if (!keyFitsInCurrentNode(key)) { dumpLeafOnMismatch("insertInCurrentNode(slice)", key); }
      assert(keyFitsInCurrentNode(key));
      assert(canInsertInCurrentNode(key, value.length()) == OP_RESULT::OK);
      cur = leaf->insert(key.data(), key.length(), value.data(), value.length());
   }
   // Debug: dump the leaf BufferFrame + node header when a key unexpectedly does
   // not fall within the leaf's fence boundaries. If magic != pid the page is
   // corrupted/wrong; if magic == pid but the key is out of range the node's
   // contents/navigation are inconsistent.
   void dumpLeafOnMismatch(const char* where, Slice key)
   {
      BufferFrame* bf = leaf.bf;
      printf("[%s] keyFitsInCurrentNode FAILED: cmpWithBoundaries=%d\n", where,
             leaf->compareKeyWithBoundaries(key.data(), key.length()));
      printf("  key: len=%u bytes=", (unsigned)key.length());
      for (u16 i = 0; i < key.length() && i < 16; i++) printf("%02x", key.data()[i]);
      printf("\n");
      if (bf) {
         printf("  bf=%p pid=%lu state=%d GSN=%lu lastWrittenGSN=%lu magic=%lu magic_end=%lu dt_id=%ld\n",
                (void*)bf, (unsigned long)bf->header.pid, (int)bf->header.state,
                (unsigned long)bf->page.GSN, (unsigned long)bf->header.lastWrittenGSN,
                (unsigned long)bf->page.magic_debugging_number,
                (unsigned long)bf->page.magic_debugging_number_end, (long)bf->page.dt_id);
      }
      printf("  node: is_leaf=%d count=%u prefix_len=%u freeSpace=%u space_used=%u "
             "lowerFence{off=%u,len=%u} upperFence{off=%u,len=%u} lowerInf=%d upperInf=%d\n",
             (int)leaf->is_leaf, (unsigned)leaf->count, (unsigned)leaf->prefix_length,
             (unsigned)leaf->freeSpace(), (unsigned)leaf->space_used,
             (unsigned)leaf->lower_fence.offset, (unsigned)leaf->lower_fence.length,
             (unsigned)leaf->upper_fence.offset, (unsigned)leaf->upper_fence.length,
             (int)leaf->isLowerFenceInfinity(), (int)leaf->isUpperFenceInfinity());
      if (u8* lf = leaf->getLowerFenceKey()) {
         printf("  lowerFenceKey=");
         for (u16 i = 0; i < leaf->lower_fence.length && i < 16; i++) printf("%02x", lf[i]);
         printf("\n");
      }
      if (u8* uf = leaf->getUpperFenceKey()) {
         printf("  upperFenceKey=");
         for (u16 i = 0; i < leaf->upper_fence.length && i < 16; i++) printf("%02x", uf[i]);
         printf("\n");
      }
      fflush(stdout);
   }
   ...
   virtual void splitForKey(Slice key)
   {
      ...
      while (true) {
         ...
         jumpmuCatch()
         {
            BACKOFF_STRATEGIES(); // mean::task::yield(); /* ::WorkerCounters::myCounters().time_counter_2++;   */
         }                       // needed in nopp config, to prevent infinite looping, if there are no free pages
      }
   }
