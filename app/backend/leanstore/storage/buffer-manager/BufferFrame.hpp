...
struct BufferFrame {
   ...
   // -------------------------------------------------------------------------------------
   // Debug: validate the persisted-page magic numbers to catch corruption around
   // the disk-I/O boundary. Both magic fields are set to the page's PID on
   // write-out (PageProviderThread::initiateWriteBack) and verified on read-in
   // (BufferManager::resolveSwip). A mismatch means the page on/from disk is not
   // what we expect (e.g. wrong LBA, DMA overrun, partial I/O).
   inline void assertPageMagicConsistent() const
   {
      ensure(page.magic_debugging_number == page.magic_debugging_number_end);
   }
   inline void assertPageMagicMatchesPid() const
   {
      ensure(page.magic_debugging_number == header.pid);
      ensure(page.magic_debugging_number_end == header.pid);
   }
   // -------------------------------------------------------------------------------------
