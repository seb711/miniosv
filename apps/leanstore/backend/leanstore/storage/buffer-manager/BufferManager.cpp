...
void BufferManager::writeAllBufferFrames()
{
   ...
   utils::Parallelize::parallelRange(dram_pool_size, [&](u64 bf_b, u64 bf_e) {
      for (u64 bf_i = bf_b; bf_i < bf_e; bf_i++) {
         ...
         // Stamp the persisted-page magic with the PID (same convention as the
         // eviction write-back path) so the read-back checks pass.
         bf.page.magic_debugging_number = bf.header.pid;
         bf.page.magic_debugging_number_end = bf.header.pid;
         bf.assertPageMagicMatchesPid();
...
void BufferManager::readPageSync(u64 pid, u8* destination)
{
   ...
   // destination points at the Page (not the BufferFrame); validate directly.
   auto& read_page = *reinterpret_cast<BufferFrame::Page*>(destination);
   ensure(read_page.magic_debugging_number == read_page.magic_debugging_number_end);
   ensure(read_page.magic_debugging_number == pid);
