...
// -------------------------------------------------------------------------------------
#if !IS_OSV
#include <execinfo.h>  // glibc backtrace; not available (and doesn't parse) on OSv
#endif
...
{
#if IS_OSV
   // glibc backtrace()/backtrace_symbols() are unavailable on OSv; the kernel
   // has its own backtrace facility for crash dumps.
#else
   void* array[10];
   size_t size;
   char** strings;
