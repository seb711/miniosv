...
{
#if IS_OSV
   // No writable filesystem / no std::ofstream on OSv (slim libc++ has no
   // <filesystem>). These file-based test-data helpers are unused by the
   // NVMe-backed YCSB benchmark.
   (void)file_name; (void)count; (void)factory;
   return false;
#else
   // Open file
