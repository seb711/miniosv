...
void LeanStore::startProfilingThread()
{
   std::thread profiling_thread([&]() {
      ...
      // -------------------------------------------------------------------------------------
      // CSV-to-disk profiling output. Disabled on OSv: the slim libc++ is built
      // without <filesystem>, so basic_ofstream is unavailable, and there is no
      // writable filesystem anyway. The console (tabulate) output below is kept.
#if !IS_OSV
