...
void delivery(Integer w_id, Integer carrier_id, Timestamp datetime)
{
   for (Integer d_id = 1; d_id <= 10; d_id++) {
      ...
      if (!is_safe_to_continue)
         continue;
      order.update1({w_id, d_id, o_id}, [&](order_t& rec) { rec.o_carrier_id = carrier_id; }, WALUpdate1(order_t, o_carrier_id));
      // -------------------------------------------------------------------------------------
...
void paymentById(Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id, Integer c_id, Timestamp h_date, Numeric h_amount, Timestamp datetime)
{
   ...
   warehouse.update1({w_id}, [&](warehouse_t& rec) { rec.w_ytd += h_amount; }, WALUpdate1(warehouse_t, w_ytd));
   Varchar<10> d_name;
   ...
   district.update1({w_id, d_id}, [&](district_t& rec) { rec.d_ytd += h_amount; }, WALUpdate1(district_t, d_ytd));
...
void analyticalQuery(Integer w_id)
{
   /*
   SELECT i.*, w.*, s.*
   FROM item i
   ...
   */
   item.scan(
       {0},
       [&](const item_t::Key& ikey, const item_t& irec) {
          if (irec.i_price > 75) {
             return true;
          }
          warehouse.scan(
              {0},
              [&](const warehouse_t::Key& wkey, const warehouse_t& wrec) {
                 stock.scan(
                     {wkey.w_id, ikey.i_id},
                     [&](const stock_t::Key& skey, const stock_t& srec) {
                        if (skey.s_i_id != ikey.i_id)
                           return true;
                        if (srec.s_quantity >= 20 || srec.s_order_cnt < 10)
                           return true;
                        return true;
                        // do stuff here but not now
                     },
                     [&]() {});
                 return true;
              },
...
void analyticalQuery2(Integer w_id)
{
   /*
   SELECT
      w.W_ID,
      ...
      d.D_ID,
      d.D_NAME,
      COUNT(o.O_ID) as recent_orders,
      SUM(o.O_OL_CNT) as total_items_ordered,
      AVG(o.O_OL_CNT) as avg_items_per_order,
      MAX(o.O_ENTRY_D) as last_order_time
   FROM
      ORDERS o
      JOIN CUSTOMER c ON (o.O_C_ID = c.C_ID AND o.O_D_ID = c.C_D_ID AND o.O_W_ID = c.C_W_ID)
